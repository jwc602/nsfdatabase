...
async function importgrant()
{
	...
	for(let j =0; j<ress.rows.length; j++)
	{
		...
		const res = await pool.query('select pid,gid,funderid,awardid from grants,publications where grants.gid = any(publications._gid) and publications.openalexid = $1',[ress.rows[j].openalexid]);
