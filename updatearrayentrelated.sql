drop function if exists public.uparrentrelated(argid text, argentrelatedid text, argentrelatedror text);
CREATE OR REPLACE function public.uparrentrelated(argid text, argentrelatedid text, argentrelatedror text)
...
BEGIN
tempdoi = argid;
myval = argentrelatedid;
myror = argentrelatedror;
select entities."EID" into eid from public.entities where openalexid = tempdoi;
select entities._relatedror into _temprors from public.entities where openalexid = tempdoi;
select entities._relatedid into _tempids from public.entities where openalexid = tempdoi;
raise info '%', _temprors;
--select array_agg(entities.rorid order by "EID") into _rorids from public.entities where levenshtein("actiontype", flatrelated) <= 1;
...
IF exists (select 1 from mytable where levenshtein(ror, myror) <=1) and exists(select 1 from mytable where levenshtein(ror, myror) > 0) THEN
	...
	raise info '%', _temprors;
	update entities set _relatedror = _temprors where openalexid = tempdoi;
...
if not exists(select 1 from entities where myval = any(_tempids)) then
	...
	update entities set _relatedid = _tempids where openalexid = tempdoi;
end if;
	return temp;
END 
$BODY$;

--select * from uparrentrelated('https://openalex.org/I1330347796','https://openalex.org/I1279970786', 'https://ror.org/01xyx5098')