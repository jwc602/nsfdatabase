--drop function if exists public.addieoblig(argrorid text, argalias varchar, argamount money, argyear int);
CREATE OR REPLACE function public.addieoblig(argrorid text, argalias varchar, argamount money, argyear int)
...
select array_agg(entities._aliases) into _1d_arr from entities where entities.rorid = argrorid;
...
IF EXISTS(SELECT 1 FROM obligations WHERE vareid =obligations.eid and argyear = obligations.year and argamount = obligations.amount) THEN
	--update obligations set name = argname, rorid = argdoi where vareid = obligations.eid and argyear = obligations.year and argamount = obligations.amount;
else
