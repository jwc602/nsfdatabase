drop function if exists public.uparrentchild(argid text, argentchildid text, argentchildror text);
CREATE OR REPLACE function public.uparrentchild(argid text, argentchildid text, argentchildror text)
...
tempdoi = argid;
myval = argentchildid;
myror = argentchildror;
select entities."EID" into eid from public.entities where openalexid = tempdoi;
select entities._childror into _temprors from public.entities where openalexid = tempdoi;
select entities._childid into _tempids from public.entities where openalexid = tempdoi;
...
IF exists (select 1 from mytable where levenshtein(ror, myror) <=1) and exists(select 1 from mytable where levenshtein(ror, myror) > 0) THEN
	...
	raise info '%', _temprors;
	update entities set _childror = _temprors where openalexid = tempdoi;
...
if not exists(select 1 from entities where myval = any(_tempids)) then
	...
	update entities set _childid = _tempids where openalexid = tempdoi;
end if;
	return _tempids;
END 
$BODY$;

--select * from uparrentchild('https://openalex.org/I1330347796','https://openalex.org/I1279970786', 'https://ror.org/01xyx5098')