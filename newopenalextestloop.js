		...
		users = fs.readFile(directoryPath + '\\' + files[filenum], function(err, data) { 
			...
			pool.on('connect', client => {
			 ...
			 /* client.query(sqlSelect,["T10112"], (err, res) => {
			  ...
			  if(users.openalex.abstract_inverted_index != null)
			  {
				let tempname = Object.keys(users.openalex.abstract_inverted_index)
				console.log(tempname[0]);
			  }
				//for(let i = 0; i < users.results.length; i++) {
					let obj = users.openalex;
					...
					if(!(Object.keys(obj).length === 0))
					{
						...
						for(let j = 0; j < obj.awards.length; j++)
						{
							...
							console.log("fileid " + files[filenum] + " resss " + ress + " obj " + obj.id);
